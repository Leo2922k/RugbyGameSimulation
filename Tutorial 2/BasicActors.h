...
namespace PhysicsEngine
{
	...
	public:
		...
		// - dimensions: 1m
		// - denisty: 1kg/m^3
		Sphere(const PxTransform& pose=PxTransform(PxIdentity), PxReal radius=2.f, PxReal density=2.f) 
			: DynamicActor(pose)
		{ 
	...
	class SSphere : public StaticActor
	{
	public:
		//a sphere with default parameters:
		// - pose in 0,0,0
		// - dimensions: 1m
		// - denisty: 1kg/m^3
		SSphere(const PxTransform& pose = PxTransform(PxIdentity), PxReal radius = 3.f, PxReal density = 1.f)
			: StaticActor(pose)
		{
			CreateShape(PxSphereGeometry(radius), density);
		}
	...
	class Rugby : public CompoundObject
	{
	public:
		Rugby(const PxTransform& pose = PxTransform(PxIdentity), PxReal radius = .2f, PxReal density = .5f)
			: CompoundObject(pose)
		{
			//CreateShape(PxSphereGeometry(radius), density);
		}
	...
	public:
		...
		// - denisty: 1kg/m^3
		Box(const PxTransform& pose=PxTransform(PxIdentity), PxVec3 dimensions=PxVec3(1.1f,1.1f,1.1f), PxReal density=1.f) 
			: DynamicActor(pose)
		{ 
	...
	class ConvexMesh : public StaticActor
	{
	public:
		//constructor
